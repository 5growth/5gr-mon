...
public class DashboardsManagerServiceImpl implements DashboardsManagerService {
  ...
  @Override
  public LogScrapperResponseWrapper createScrapper(LogScrapperRequestWrapper scrapperRequestWrapper) {
    LogScrapperResponseWrapper response = new LogScrapperResponseWrapper();

    Optional<Log> logOP = storageService.getLog(scrapperRequestWrapper.getNsid());
    if (logOP.isPresent()){
      String scrapperId = elasticsearchService.startLogScrapper(scrapperRequestWrapper);
      response = scrapperRequestToScrapperResponse(scrapperRequestWrapper, scrapperId);
      return response;
    } else {
      log.warn("Topic not present in DB {}", scrapperRequestWrapper.getNsid());
      throw new RuntimeException("Topic not present in DB" + scrapperRequestWrapper.getNsid());
    }
  }

  @Override
  public JSONObject deleteScrapper(String scraperId){
    JSONObject node = new JSONObject();
    boolean deleted = elasticsearchService.deleteLogScrapper(scraperId);
    if (deleted == true){
      JSONArray append = new JSONArray();
      append.appendElement(scraperId);
      node.appendField("deleted", append);
    } else {
      node.put("Deleted", "error removing logScrapper");
    }
    return node;
  }
  ...
  private LogScrapperResponseWrapper scrapperRequestToScrapperResponse(LogScrapperRequestWrapper requestWrapper, String scrapperId){
    LogScrapperResponseWrapper responseWrapper = new LogScrapperResponseWrapper();
    responseWrapper.setScraper_id(scrapperId);
    responseWrapper.setExpression(requestWrapper.getExpression());
    responseWrapper.setInterval(requestWrapper.getInterval());
    responseWrapper.setKafkaTopic(requestWrapper.getKafkaTopic());
    responseWrapper.setNsid(requestWrapper.getNsid());
    responseWrapper.setVnfid(requestWrapper.getVnfid());
    responseWrapper.setPerformanceMetric(requestWrapper.getPerformanceMetric());
