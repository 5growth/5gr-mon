...
def kafka_consumer_refresh_dashboard_handler(topic):
    ...
    time.sleep(5)
    # NEW: kafka consumer does not create dashboard, only refreshes it

    # This call is now needed since the dashboard is created beforehand.
    #time.sleep(2)            
    logger.info("Refreshing dashboard for %s topic", topic)
    logger.info("Configuring dashboard index-pattern for topic: %s", topic)
    subprocess.call(['/bin/bash', '/usr/bin/kafka_consumer/refresh_dashboard.sh', topic, kibana_ip_port])
...
def kafka_consumer():
    ...
    try:
        ...
        # value not needed any more
        topic = data["topic"]
        

        # Create Kafka consumer to wait for the first message received in the topic and, then, refresh the dashboard.
        # value not needed any more
        thread = threading.Thread(target = kafka_consumer_refresh_dashboard_handler, args = [topic])
        thread.start()
...
if __name__ == "__main__":
    # Usage: /usr/bin/python3 kafka_consumer.py --kibana_ip_port localhost:8080 --kafka_ip_port localhost:9092 --log info
    ...
    global kibana_ip_port 
    kibana_ip_port = str(args.kibana_ip_port)
