...
public class PrometheusConnector {
    ...
    public PrometheusConnector(
        ...
        log.info("Looking for calculate rules at {}", calculateRulesPath);
        calculateRules = new File(calculateRulesPath);
        if (!calculateRules.isFile()) {
            throw new IllegalArgumentException(String.format(
                    "Illegal alert rules path '%s'",
                    calculateRulesPath
            ));
        }
        log.info("Calculate rules found");
                ...
                try {
                    ...
                    if (!done) {
                        future.fail(new IllegalStateException(
                                "Error reloading prometheus config: sendToPrometheusGateway timed out"
                        ));
        ...
        try {
            for (String promName: new String[]{"prometheus", "prometheus-core", "prometheus-server"}) {
                ...
                try {
                    boolean done = ps.waitFor(1000, TimeUnit.MILLISECONDS);
                    if (!done) {
                        log.warn("Error fetching prometheus service name: sendToPrometheusGateway timed out");
                        return null;
                    }
                    if (ps.exitValue() == 0) {
                        return promName;
            ...
            try {
                ...
                if (!done) {
                    throw new IllegalStateException(
                            "Error fetching prometheus PID: sendToPrometheusGateway timed out"
                    );
                }
                ...
                } else throw new IllegalStateException("Could not find prometheus sendToPrometheusGateway");
            } catch (InterruptedException exc) {
    ...
    public CalculateRules getCalculateRules() {
        try {
            byte[] readRules = Files.readAllBytes(calculateRules.toPath());
            if (log.isTraceEnabled()) {
                log.trace("Reading Calculate Rules config:\n");
                log.trace(new String(readRules));
            }
            return mapper.readValue(readRules, CalculateRules.class);
        } catch (IOException e) {
            throw new IllegalStateException("Could not read Calculate rules file", e);
        }
    }
    ...
    public Future<Void> setCalculateRules(CalculateRules newRules) {
        try {
            byte[] writtenRules = mapper.writeValueAsBytes(newRules);
            if (log.isTraceEnabled()) {
                log.trace("Writing Calculate Rules config:\n");
                log.trace(new String(writtenRules));
            }
            Files.write(calculateRules.toPath(), writtenRules);
            return reloadPrometheusConfig();
        } catch (JsonProcessingException e) {
            log.debug("Illegal calculate rules provided: {}", e.getMessage());
            throw new IllegalArgumentException("Provided calculate rules are illegal", e);
        } catch (IOException e) {
            throw new IllegalStateException("Could not write rules file", e);
        }
    }
