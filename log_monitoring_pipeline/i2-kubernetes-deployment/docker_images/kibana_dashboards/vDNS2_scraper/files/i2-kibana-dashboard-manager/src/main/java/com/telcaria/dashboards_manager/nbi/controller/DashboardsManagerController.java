...
import com.telcaria.dashboards_manager.nbi.wrapper.*;
...
public class DashboardsManagerController {
  ...
  @PostMapping(value = "/logScraper")
  public @ResponseBody ResponseEntity getDataFromTopic(@RequestBody LogScrapperRequestWrapper scrapperRequestwrappper) {
    try {
      LogScrapperResponseWrapper response = dashboardsManagerService.createScrapper(scrapperRequestwrappper);
  ...
  @DeleteMapping(value = "/logScraper/{scrapperId}")
  public @ResponseBody ResponseEntity deleteScrapper(@PathVariable("scrapperId") String scrapperId) {
    // ToDo Code all endpoint
    JSONObject response;
    try {
      response = dashboardsManagerService.deleteScrapper(scrapperId);
      return new ResponseEntity(response, HttpStatus.ACCEPTED);
    } catch (Exception e){
      return new ResponseEntity(e.toString(), HttpStatus.BAD_REQUEST);
    }
